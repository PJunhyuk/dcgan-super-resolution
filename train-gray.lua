...
-- nc x 64 x 64
netG:add(SpatialFullConvolution(nc, ngf*4, 2, 2, 2, 2))
netG:add(SpatialBatchNormalization(ngf*4))
-- ngf*4 x 128 x 128
netG:add(SpatialFullConvolution(ngf*4, ngf*2, 2, 2, 2, 2))
...
-- set testSample function for test
function testSample(real_none_color_sample)
    ...
    for i = 1, opt.fineSize/2 do
        for j = 1, opt.fineSize/2 do
            ...
            real_none_2x_sample[{ {2*i-1}, {2*j} }] = real_reduced_sample[{ {i}, {j} }]
            real_none_2x_sample[{ {2*i}, {2*j-1} }] = real_reduced_sample[{ {i}, {j} }]
...
-- test
local real_none_color_test = torch.Tensor(3, opt.fineSize, opt.fineSize)
real_none_color_test = data:getBatch()[1]

testSample(real_none_color_test)
